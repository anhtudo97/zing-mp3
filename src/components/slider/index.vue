...
export default {
  ...
  setup(props) {
    ...
    const handleNext = () => {
      if (currentIndex.value < sliders.length - 1) currentIndex.value++
      else currentIndex.value = 0
    }

    const handlePrev = () => {
      if (currentIndex.value > 0) currentIndex.value--
      else currentIndex.value = sliders.length - 1
    }

    const autoPlayCarousel = (duration: number): void => {
      setTimeout(() => {
        if (!isPause.value) handleNext()
        autoPlayCarousel(duration)
      }, duration)
    }

    const handlePause = (e: MouseEvent): void => {
      if (e.type === 'mouseover') isPause.value = true
      if (e.type === 'mouseleave') isPause.value = false
    }
