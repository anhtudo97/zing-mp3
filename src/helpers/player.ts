...
class Player {
  ...
  constructor() {
    ...
    if (obj) this._state = reactive(obj)
  ...
  set volume(val: number) {
    if (val !== 0) this._volumeBeforeMuted = val

    this._state.volume = val
    if (this._howler) this._howler.volume(val / 100)
  }
  ...
  set isMuted(val: boolean) {
    ...
    else this.volume = this._volumeBeforeMuted
  ...
  set currentSong(song: Song) {
    ...
    if (index >= 0) this.recentItems.splice(index, 1)
  ...
  set isPlaying(val: boolean) {
    ...
    if (this._howler) {
      if (val) this._howler.play()
      else this._howler.pause()
    } else if (this.currentSong) this.loadSong(true)
  }
  ...
  set isShuffle(val: boolean) {
    ...
    else this.queues = this.currentPlaylistItems
  }
  ...
  // Update new meta data on new song is played
  ...
  calculateDuration(percent: number) {
    if (this._howler) return (percent / 100) * this._howler.duration() || 0
  ...
  playPlaylist(playlist = <Playlist>{}, isShuffle = false) {
    if (playlist.encodeId === this.currentPlaylistId) return this.togglePlay()

    if (playlist.song && Array.isArray(playlist.song.items)) {
      const firstSong = playlist.song.items[0]
      if (firstSong) this.playSong(firstSong, playlist, isShuffle)
